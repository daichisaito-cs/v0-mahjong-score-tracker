...
export function DashboardContent({ displayName, results, rollups, yakumanRecords = [] }: DashboardContentProps) {
  ...
  const pointsHistory = filteredResults.map((result, index) => {
    const cumulativePoints =
      filteredResults.slice(0, index + 1).reduce((sum, r) => sum + Number(r.point), 0) + rolledTotalPoints
    return {
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      <div>
        <h1 className="text-2xl font-bold text-foreground">{displayName || "ユーザー"}さん、こんにちは</h1>
          ...
          <CardContent className="pt-0 pb-2 !px-2 sm:!px-4">
            <ResponsiveContainer width="100%" height={205}>
              <LineChart data={pointsHistory} margin={{ top: 6, right: 10, bottom: 6, left: 0 }}>
                ...
                <XAxis dataKey="game" stroke="#9ca3af" fontSize={12} tickLine={false} axisLine={{ stroke: "#e5e7eb" }} />
                <YAxis width={32} stroke="#9ca3af" fontSize={12} tickLine={false} axisLine={{ stroke: "#e5e7eb" }} />
                <Tooltip
      ...
      <YakumanCard records={yakumanRecords.filter((r) => r.gameType === gameType)} />
    </div>
  )
}
